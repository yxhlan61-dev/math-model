...
为避免不同剩余时域造成的偏差，按相同目标 6 小时区段比较各发布时点可用预报的平均绝对误差，结果如图~\ref{fig:q3_pv_err} 所示，每组样本数均为 13140 个 10 分钟时段。以下误差在插值和换算前的原始光伏功率口径下统计，单位为 kW；进入调度模型前，预报与实际功率均统一乘以 $\Delta t=1/6$ 转为 kWh。6:00--12:00 区段上，0:00 与 6:00 预报的 MAE 分别为 331.067 与 180.934 kW；12:00--18:00 区段上，0:00、6:00 与 12:00 预报分别为 416.174、273.430 与 136.534 kW；18:00--24:00 区段上四次预报依次为 9.721、9.263、9.267 与 9.165 kW。可见 6:00 与 12:00 的更新明显降低了同目标白天时段的预测误差，而 18:00--24:00 的光伏出力基本为零，各次预报误差均很小，18:00 更新所提供的光伏信息增益十分有限。
...
在 334 个正式日上，四种策略的实际总费用依次为 14861411.71、14653421.12、14513256.10 与 14510355.99 元。全部四时点滚动相对仅 0:00 策略节省 351055.72 元（2.36\%），紧急购电量减少 25.40\%。图~\ref{fig:q3_marginal} 表明，6:00 与 12:00 更新贡献主要收益，18:00 的边际收益很小。
...
6:00、12:00、18:00 更新分别在 238、318、102 个正式日改变最终购电量；因此，问题三以 6:00 与 12:00 的新预报为主要决策信息，18:00 更新作为收益为正的补充修正。

\subsubsection{全部四时点策略的费用汇总}

正式提交采用全部四时点策略：全年实际总费用为 14510355.99 元，其中紧急购电费为 850178.36 元（5.86\%），仍是后续改进的主要可控成本。

\input{sections/tables_q3_dates}
