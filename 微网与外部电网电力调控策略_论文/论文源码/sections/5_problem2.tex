...
问题二要求在每天 0:00 一次性确定当天 144 个时段的计划购电量与储能充放电策略，而当天的小区负载与光伏发电功率在计划时刻尚未揭晓，实际运行中出现电力缺口时只能以 5 倍电价紧急购电。本文采用``周期加权平均预测 + 24 小时联合残差场景 + 季节余弦终端软区间混合整数线性规划''的技术路线：每天 0:00 仅利用该日之前已发生的实际数据分别预测负载与光伏，从历史样本外残差中整日联合抽样生成场景，再求解以计划购电费、期望紧急购电费与终端储电量偏离惩罚之和为目标的单日混合整数线性规划；当天实际值揭晓后回放结算，并将日末储电量传递给次日。正式输出区间为 2025 年 2 月 1 日至 12 月 31 日，1 月用于构造残差场景库与风险调整的历史窗口。
...
在 2025 年 2 月 1 日至 12 月 31 日共 334 天、48096 个 10 分钟时段上，逐日向前预测的结果为：负荷平均绝对误差 29.902 kWh、均方根误差 41.701 kWh、加权平均绝对百分比误差 3.89\%；光伏全时段平均绝对误差 24.970 kWh、均方根误差 50.047 kWh、WAPE 6.30\%，非零时段平均绝对误差 44.669 kWh。所有误差均来自严格按时间向前的样本外预测，不是全年拟合后的样本内结果。以 2025 年 3 月 20 日为例，负荷与光伏的实际值和预测值对比如图~\ref{fig:q2_day} 所示：负荷的预测曲线与实际曲线基本重合，光伏在午间的预测峰值略低于实际值，说明周期加权预测能够刻画日内的峰谷结构。
...
将 3 月各日的 144 个时段汇总为日总电量，预测与实际情况如图~\ref{fig:q2_month} 所示：月内负荷呈明显的星期周期波动，光伏发电水平缓慢上升，预测偏差主要出现在光伏出力快速变化的时段。
...
\input{sections/tables_q2_dates}

\subsubsection{终端储电量机制的对照分析}

为验证季节余弦基准与因果风险微调的作用，本文在同一预测、同一随机种子与同一联合残差场景下，对四种终端处理方式做了对照计算：完全不设终端约束时，模型会把日末储电量压到下限 1200 kWh，全年实际总费用为 14651641.38 元，为四者最高；引入固定 6000 kWh 基准后费用降至 14640006.24 元，节省 11635.14 元；改用季节余弦基准、暂不叠加风险微调，费用进一步降至 14637301.67 元，再节省 2704.57 元，原因是基准随季节移动，避免了冬夏两季共用同一目标带来的次优；在季节基准上叠加因果风险微调后，全年实际总费用为 14637169.37 元，再节省 132.30 元，为四者最低。可见终端价值的刻画精度每提高一步都能带来可观的费用下降，季节余弦基准的贡献远大于风险微调。

风险微调在正式期共上调 189 天、下调 145 天，其中 16 天触及 $+600$ kWh 的截断上限，未出现 $-600$ kWh 的截断；调整量均值为 67.15 kWh、标准差为 248.36 kWh，且风险分数高于历史中位数的日期均为正调整、低于中位数的均为负调整，符号无违例。

值得注意的是，正式期 334 天的日末储电量全部贴在``当日微调后''软区间的下沿，日末储电量范围为 5105.04$\sim$7196.85 kWh。这表明在单日有限时域模型中，终端价值主要表现为一条可因果移动的季节性最低备用曲线，风险微调的作用是把这条曲线按当日场景风险上下平移，而不是让储电量在软区间内部自由取最优。四种终端处理方式的费用与储电量对比如图~\ref{fig:q2_reserve} 所示。
