...
设 $\lambda_{d,t}$ 为第 $d$ 天第 $t$ 个 10 分钟时段的实际电价（元/kWh），$d=1,\dots,365$，$t=1,\dots,144$。按星期分类求平均，可得各类星期的典型电价曲线，如图~\ref{fig:q4_curves} 所示。由图可见，日内分时电价呈稳定的阶梯结构，工作日与周末的价格水平存在明显差异。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\textwidth]{../figures/问题四/问题四_典型星期电价曲线.pdf}
  \caption{星期一至星期日的典型电价曲线}
  \label{fig:q4_curves}
\end{figure}
...
式中：$x_n$ 为第 $n$ 个 10 分钟时段的电价（元/kWh）；$\bar x$ 为序列均值；$k$ 为滞后阶数。一天包含 144 个 10 分钟时段，因此第 $h$ 个整日滞后的自相关系数即 $\widehat\rho(144h)$。计算结果如图~\ref{fig:q4_acf} 所示：第 1、7 个整日滞后分别为 0.9299 与 0.9576，第 14、21、28 个整日滞后分别为 0.9260、0.8947 与 0.8634，均明显高于其相邻的非整周滞后，形成整周局部峰值。据此，把过去 4 个同星期、同时刻的价格作为 0:00 全天预测的输入变量。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\textwidth]{../figures/问题四/问题四_ACF自相关.pdf}
  \caption{电价的整日滞后自相关函数}
  \label{fig:q4_acf}
\end{figure}
...
该模型的样本外预测精度为：平均绝对误差 0.0467 元/kWh，均方根误差 0.0665 元/kWh，WAPE 6.17\%，日内 Spearman 秩相关系数 0.9526，前 20\% 高价时段识别率 89.30\%，最高价时刻误差 119.6 min。作为对照，上一日同期、上一周同期与最近 4 周简单平均模型的平均绝对误差分别为 0.0843、0.0472 与 0.0489 元/kWh，日内 Spearman 相关系数分别为 0.9380、0.9460 与 0.9517。与最优的对照模型（上一周同期）相比，正式模型的平均绝对误差降低约 0.91\%，WAPE 由 6.22\% 降至 6.17\%，日内排序相关系数由 0.9460 提高到 0.9526。由于储能调度更依赖价格的日内峰谷顺序，本文选择排序稳定性更好的四周指数加权模型。各模型误差对比如图~\ref{fig:q4_pred_err} 所示。
...
四种滚动策略的实际总费用分别为 15778106.07、15553894.38、15397581.11 与 15393286.54 元，实际紧急购电量分别为 322925.46、245602.58、233774.31 与 233383.17 kWh。全部四时点策略相对仅 0:00 策略节省 384819.53 元，降幅约 2.44\%；紧急购电量减少 89542.29 kWh，降幅约 27.73\%。其中 6:00 更新节省 224211.69 元，12:00 更新节省 156313.27 元，18:00 更新仅节省 4294.57 元，可见 6:00 与 12:00 贡献了绝大部分经济收益，结论与问题三一致。

正式 4-3 策略全年 0:00 原计划购电量为 22484536.18~kWh，滚动调整后的最终购电量为 21972266.81~kWh，实际紧急购电量为 233383.17~kWh。按净结算口径，计划与调整结算费为 14462432.65 元，紧急购电费为 930853.89 元，实际总费用为 15393286.54 元；后者为前两项之和。正式 4-3 相对 4-2 节省 136228.24 元（0.88\%），紧急购电量减少 71871.30~kWh（23.55\%），说明在波动电价下日内滚动调整同样能带来可观收益。

\input{sections/tables_q4_dates}
